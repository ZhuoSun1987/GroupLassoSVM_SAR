function [ SumCost,CostParts ] =CostAll(W,DataOpt,CostFuncOpt,SparseOption) 
%UNTITLED8 Summary of this function goes here
%   Detailed explanation goes here
%
%   this function is used to compute the cost of a Zhuo's SVM model at the
%   given paremeter W
%
...
if CostFuncOpt.LambdaList(3)>0
    Cost=(W(1:end-1)')*DataOpt.Q*W(1:end-1);
    CostParts(3)=Cost;
end

%% for the sparsity term
if CostFuncOpt.LambdaList(4)>0
    [ Cost ] = SparsityCost( W(1:end-1),SparseOption );
    CostParts(4)=Cost;
end

%% compute the sum of all the terms
CostParts=reshape(CostParts,[4,1]);
LambdaList=reshape(CostFuncOpt.LambdaList,[4,1]);
SumCost=sum(CostParts.* LambdaList);
