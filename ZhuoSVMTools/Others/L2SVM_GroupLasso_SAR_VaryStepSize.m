function [ W,LOG ] = L2SVM_GroupLasso_SAR_VaryStepSize( X,Y,W0,GroupIndListArray,GroupWeightList,LL,LambdaList,q,Option )
...
CostW=Cost1+Cost2+LambdaList(3)*Cost3;
if Print==1
...
for Iter=1:MaxIter
    ...
    AfterProxiamlCostList(Iter,:)=[Cost1,Cost2,LambdaList(3)*Cost3,CostW];
    ...
    CostSubjects=max(0,1-(X*ProxW).*Y);
    Cost1ProxW=mean(CostSubjects.^SVMNorm);
    Cost2ProxW=0;
    if NeedQPart==1
        Cost2ProxW=(ProxW(1:end-1)')*Q*ProxW(1:end-1);
    end
    SparseCostProxW=GroupWeightList*cellfun(@(GroupIndList) norm(ProxW(GroupIndList),q),GroupIndListArray);
    QFun_ProxW=SmoothCostW+Grad'*(ProxW-W)+(ProxW-W)'*(ProxW-W)/(2*StepSize)+LambdaList(3)*SparseCostProxW;
    F_ProxW   =Cost1ProxW+Cost2ProxW+LambdaList(3)*SparseCostProxW;
